...
`master`; unchanged projections create no commit, including when a re-fetch
would only stamp new retrieval timestamps. It then starts the existing
...
Unchanged data, including equal source SHA-256 / last-modified whose public
index would otherwise only change `retrieved_at`, creates no commit and, when
the hosted descriptor is current, no publication run. A stale hosted copy is
reconciled automatically on the next run. Validation failures and concurrent
`master` updates stop before a push.
...
[browser distribution](docs/browser-delivery.md) adds latest discovery and verified
Pages publication; production app adoption remains separate.
